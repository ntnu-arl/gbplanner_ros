...
MapManagerVoxblox<SDFServerType, SDFVoxelType>::getBoxStatus(
  ...
  // else if(box_status_method_ == 3)
  // {
  //   float voxel_size = sdf_layer_->voxel_size();
  //   float voxel_size_inv = 1.0 / voxel_size;

  //   voxblox::LongIndex end_voxel_index =
  //       voxblox::getGridIndexFromPoint<voxblox::LongIndex>(
  //           (center - size/2.0).cast<voxblox::FloatingPoint>(), voxel_size_inv);
  ...
  //   voxblox::LongIndex voxel_index = end_voxel_index;
  //   for (voxel_index.x() = end_voxel_index.x();
  //       voxel_index.x() < box_center.x() + std::ceil(box_voxels.x() / 2.0);
  //       voxel_index.x()++) {
  //     for (voxel_index.y() = end_voxel_index.y() - box_voxels.y() / 2;
  //         voxel_index.y() < box_center.y() + std::ceil(box_voxels.y() / 2.0);
  //         voxel_index.y()++) {
  //       for (voxel_index.z() = end_voxel_index.z() - box_voxels.z() / 2;
...
MapManagerVoxblox<SDFServerType, SDFVoxelType>::getPathStatus(
  ...
  if(line_status_method_ == 1) {
    ...
    start_e = start;
    // int check_count = std::ceil(edge_vec.norm() * voxel_size_inv);
    int check_count = std::ceil(edge_vec.norm() * box_size.maxCoeff());
    for (int i = 0; i < check_count; i++) {
      Eigen::Vector3d vox_center = start_e + i * edge_vec / edge_vec.norm();
