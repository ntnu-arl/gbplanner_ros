...
inline void convert(const Eigen::Vector4d &st, geometry_msgs::Pose &p)
{
	tf::Quaternion quat;
...
inline void convert(const geometry_msgs::Pose &p, Eigen::Vector4d &st)
{
	st[0] = p.position.x;
	st[1] = p.position.y;
	st[2] = p.position.z;
...
inline void convert(const std::vector<geometry_msgs::Pose> &pose_path, std::vector<Eigen::Vector4d> &vec_path)
{
	vec_path.clear();
...
inline void convert(const std::vector<Eigen::Vector4d> &vec_path, std::vector<geometry_msgs::Pose> &pose_path)
{
	pose_path.clear();
...
inline void linearlyInterpolateYaw(std::vector<geometry_msgs::Pose> &path)  // Keeps the yaw same for first and last pose, and interpolates the rest
{
	...
	Eigen::Vector4d first_point, last_point;
